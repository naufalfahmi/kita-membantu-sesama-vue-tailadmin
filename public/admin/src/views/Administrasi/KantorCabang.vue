<template>
  <AdminLayout>
    ...
    <ConfirmModal
      ...
      message="Apakah Anda yakin ingin menghapus kantor cabang ini? Tindakan ini tidak dapat dibatalkan."
      confirmText="Hapus"
...
const fetchData = async () => {
  ...
  try {
    ...
    if (filterNamaKantor.value) {
      searchParams.append('search', filterNamaKantor.value)
    }
    
    const url = `/admin/api/kantor-cabang${searchParams.toString() ? '?' + searchParams.toString() : ''}`
...
const handleAdd = () => {
  router.push('/administrasi/kantor-cabang/new')
}

// Handle edit - redirect to form page
const handleEdit = (id: string) => {
  router.push(`/administrasi/kantor-cabang/${id}/edit`)
}
...
const confirmDelete = async () => {
  ...
  try {
    const csrfToken = getCsrfToken()
    const response = await fetch(`/admin/api/kantor-cabang/${deleteId.value}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
        'X-Requested-With': 'XMLHttpRequest',
    ...
    if (result.success) {
      ...
      toast.error(result.message || 'Gagal menghapus kantor cabang')
    }
  } catch (error) {
    console.error('Error deleting:', error)
    toast.error('Terjadi kesalahan saat menghapus kantor cabang')
  } finally {
