        ...
        <div class="flex gap-4">
          <div class="flex-1">
            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
              Cari Nama Kegiatan / Kota
            </label>
            <input
              type="text"
              v-model="filterSearch"
              placeholder="Cari nama kegiatan atau kota..."
...
const columnDefs = [
  ...
  {
    headerName: 'Tanggal Dilaksanakan',
    field: 'activity_date',
    sortable: true,
    flex: 1,
...
const fetchData = async () => {
  loading.value = true
  try {
    const response = await fetch('/admin/api/landing-kegiatan', {
      method: 'GET',
      headers: {
...
const handleAdd = () => {
  router.push('/company/landing-kegiatan/new')
}

// Handle edit - redirect to form page
const handleEdit = (id: string | number) => {
  router.push(`/company/landing-kegiatan/${id}/edit`)
}
