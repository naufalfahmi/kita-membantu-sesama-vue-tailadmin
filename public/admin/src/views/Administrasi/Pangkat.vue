...
<template>
  <AdminLayout>
    ...
    <ConfirmModal
      ...
      title="Hapus Pangkat"
      message="Apakah Anda yakin ingin menghapus pangkat ini? Tindakan ini tidak dapat dibatalkan."
      confirmText="Hapus"
...
const currentPageTitle = computed(() => (route.meta.title as string) || 'Pangkat')
const toast = useToast()
const { fetchUser, hasPermission, isAdmin } = useAuth()
const canCreate = computed(() => isAdmin() || hasPermission('create pangkat'))
const canUpdate = computed(() => isAdmin() || hasPermission('update pangkat'))
const canDelete = computed(() => isAdmin() || hasPermission('delete pangkat'))
const canView = computed(() => isAdmin() || hasPermission('view pangkat'))

// Column definitions
const columnDefs = [
  {
    headerName: 'Nama Pangkat',
    field: 'nama',
    sortable: true,
    flex: 1,
  },
...
const fetchData = async () => {
  ...
  try {
    ...
    if (filterNamaPangkat.value) {
      searchParams.append('search', filterNamaPangkat.value)
    }

    const url = `/admin/api/pangkat${searchParams.toString() ? '?' + searchParams.toString() : ''}`
    ...
    if (result.success) {
      ...
      console.error('Failed to fetch pangkat data:', result.message)
      rowData.value = []
...
const handleAdd = () => {
  router.push('/administrasi/pangkat/new')
}

// Handle edit - redirect to form page
const handleEdit = (id: string) => {
  router.push(`/administrasi/pangkat/${id}/edit`)
}
...
const confirmDelete = async () => {
  ...
  try {
    const csrfToken = getCsrfToken()
    const response = await fetch(`/admin/api/pangkat/${deleteId.value}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
        'X-Requested-With': 'XMLHttpRequest',
    ...
    if (result.success) {
      ...
      toast.error(result.message || 'Gagal menghapus pangkat')
    }
  } catch (error) {
    console.error('Error deleting:', error)
    toast.error('Terjadi kesalahan saat menghapus pangkat')
  } finally {
...
// Filter state
const filterNamaPangkat = ref('')

// Filtered data for AG Grid
const gridRowData = computed(() => {
  let filtered = [...(rowData.value || [])]
  // Filter by nama pangkat
  if (filterNamaPangkat.value) {
    filtered = filtered.filter((item) => item.nama && item.nama.toLowerCase().includes(filterNamaPangkat.value.toLowerCase()))
  }
  return filtered
})
