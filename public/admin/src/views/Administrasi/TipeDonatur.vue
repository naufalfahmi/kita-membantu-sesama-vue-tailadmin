<template>
  <AdminLayout>
      ...
      <div class="mb-6 rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="flex gap-4">
          <div class="flex-1">
            ...
            <input
              type="text"
              v-model="filterTipeDonatur"
              placeholder="Cari tipe donatur..."
    ...
    <ConfirmModal
      ...
      title="Hapus Tipe Donatur"
      message="Apakah Anda yakin ingin menghapus tipe donatur ini? Tindakan ini tidak dapat dibatalkan."
      confirmText="Hapus"
...
const canCreate = computed(() => hasPermission('create tipe donatur'))
const canUpdate = computed(() => hasPermission('update tipe donatur'))
const canDelete = computed(() => hasPermission('delete tipe donatur'))

const showDeleteModal = ref(false)
const deleteId = ref<string | null>(null)
const filterTipeDonatur = ref('')
const rowData = ref<any[]>([])

const columnDefs = [
  {
    headerName: 'Tipe Donatur',
    field: 'nama',
    sortable: true,
    flex: 1,
  },
...
const fetchData = async () => {
  try {
    ...
    if (filterTipeDonatur.value) {
      searchParams.append('search', filterTipeDonatur.value)
    }

    const url = `/admin/api/tipe-donatur${searchParams.toString() ? '?' + searchParams.toString() : ''}`
    ...
    if (result.success) {
      ...
      rowData.value = []
      toast.error(result.message || 'Gagal memuat data tipe donatur')
    }
  } catch (error) {
    console.error('Error fetching tipe donatur:', error)
    rowData.value = []
...
const handleAdd = () => {
  router.push('/administrasi/tipe-donatur/new')
}

const handleEdit = (id: string) => {
  router.push(`/administrasi/tipe-donatur/${id}/edit`)
}
...
const confirmDelete = async () => {
  ...
  try {
    const csrfToken = getCsrfToken()
    const response = await fetch(`/admin/api/tipe-donatur/${deleteId.value}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
        'X-Requested-With': 'XMLHttpRequest',
    ...
    const result = await response.json()
    if (result.success) {
      toast.success('Tipe donatur berhasil dihapus')
      await fetchData()
    } else {
      toast.error(result.message || 'Gagal menghapus tipe donatur')
    }
  } catch (error) {
    console.error('Error deleting tipe donatur:', error)
    toast.error('Terjadi kesalahan saat menghapus tipe donatur')
  } finally {
