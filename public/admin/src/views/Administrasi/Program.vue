...
const canCreate = computed(() => hasPermission('create program'))
const canUpdate = computed(() => hasPermission('update program'))
const canDelete = computed(() => hasPermission('delete program'))
...
const columnDefs = [
  ...
  },
  {
    ...
    cellRenderer: (params: any) => {
      ...
      `
      if (canUpdate.value) editBtn.onclick = () => handleEdit(params.data.id)
      ...
      `
      if (canDelete.value) deleteBtn.onclick = () => handleDelete(params.data.id)
...
const handleAdd = () => {
  router.push('/administrasi/program/new')
}

// Handle edit - redirect to form page
const handleEdit = (id: string) => {
  router.push(`/administrasi/program/${id}/edit`)
}
...
const confirmDelete = async () => {
  ...
  try {
    const csrfToken = getCsrfToken()
    const response = await fetch(`/admin/api/program/${deleteId.value}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
        'X-Requested-With': 'XMLHttpRequest',
